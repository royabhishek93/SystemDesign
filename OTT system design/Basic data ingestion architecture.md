...
System design is a very broad topic. There are many ways to approach a design problem and there is no one right answer. It is important to understand the requirements and constraints of the problem. It is also important to understand the trade-offs between different approaches. Thus, a systematic approach is required to solve a design problem. The following is a general approach to solve a design problem:

Gather requirements - Understand the problem and the requirements.
Clarity - Ask questions to clarify the requirements.
Scope the problem - Identify and separate the important requirements from the minor ones. Identify non-functional requirements and do some back of the envelope calculations to get an idea of the scale of the problem.
Design - Create a technical design document that describes the architecture and the components of the system.
What is an OTT?
...
Revenue in the OTT Video segment is projected to reach $275 Billion in 2022. In the OTT Video segment, the number of users is expected to amount to 3,508M users by 2027. User penetration will be 35.7% in 2022 and is expected to hit 44.1% by 2027. The average revenue per user (ARPU) in the OTT Video segment is projected to amount to US$101.20 in 2022.

What are the requirements of an OTT?
There are numerous stakeholders in an OTT. The following are the main stakeholders:

Content providers - Content providers are the ones who provide the content to the OTT. They are the ones who own the content. They are the ones who decide what content to provide to the OTT.
OTT - The OTT is the one who provides the content to the users. The OTT is the one who decides how to provide the content to the users.
Users - The users are the ones who consume the content provided by the OTT. The users are the ones who decide what content to consume.
User requirements
Playback - The users should be able to play the content provided by the OTT.
Search - The users should be able to search for content provided by the OTT.
Recommendations - The users should be able to get recommendations for content provided by the OTT.
Personalization - The users should be able to personalize their experience with the OTT.
Content provider requirements
Upload - The content providers should be able to upload content to the OTT.
Distribution - The content providers should be able to distribute content to users.
Analytics - The content providers should be able to get analytics about the content they provide to the OTT.
...
Hence, the first API that we would need to build is an API to upload content. Following are some data points that we would like to capture about the content:
...
metadata - 4KB
creator_id - 4 bytes
So the total size of the metadata is around 5KB. The total size of the metadata for 100,000 titles is 5KB * 100,000 = 500 MB. The amount of data that we would like to store is very small and can be stored on the same machine. Hence, we can use a relational database to store the metadata. We would like to use a database that can also store JSON data. Hence, we can use PostgreSQL.

Quick Recap

Client requests to upload content to the OTT.
Server stores metadata in the database and generates a presigned URL. The server sends the presigned URL to the client.
Client uploads the content to the file storage using the presigned URL and multipart upload.
Client sends a request to the server to update the metadata on successful upload.
...
Client uploads the content to the file storage using the presigned URL and multipart upload.
Client sends a request to the server to update the metadata on successful upload.
Server sends a message to the message queue to process the content.
...
Streaming content to the users The overall process of streaming content to the users is as follows:
...
Client uploads the content to the file storage using the presigned URL and multipart upload.
Client sends a request to the server to update the metadata on successful upload.
Server sends a message to the message queue to process the content for each format and resolution.
The content is processed asynchronously by workers that compress, transcode, crop and package the content.
The content is finally uploaded to S3 and metadata tables are updated.
Content streaming
Client requests to play a particular title.
Server sends the details of the chunks to the client.
The data can be served either from the server or a CDN.
Client downloads the chunks as the user watches the content.
Clients can also seek to a particular point in the content. The client can download the chunk that contains the point that the client wants to seek to.